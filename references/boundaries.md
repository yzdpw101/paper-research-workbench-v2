...
- `--idx` 依赖搜索结果排序，搜索结果受万方服务端会话影响；同一关键词不同会话返回顺序可能不同。建议使用精确关键词（如论文标题）确保命中
- 并发调用 `ieee-download.js` 可能导致文件串扰；多篇下载请使用 `parallel-download.js` 或串行调用
- `--save-as` 路径相对于 skill 目录；建议使用绝对路径（如 `E:/Desktop/paper.pdf`）
