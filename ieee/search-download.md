...
v2 uses `browser-launcher.js` + `config.js` — no separate `init.js` needed. ${SKILL_DIR} resolves automatically in Reasonix. Run `set-browser.js` once if `.state/.setup-done` is missing.
...
2. User says "摘要" → add --no-snippet (omit to include abstracts) → show snippets
...
node "${SKILL_DIR}/scripts/ieee-search.js" \
  ...
  --rows 25 \
  --page 1 \
  --no-snippet (omit to include abstracts)
```
...
- `--no-snippet (omit to include abstracts)` : Expand abstracts; each item gets a `.snippet` field
