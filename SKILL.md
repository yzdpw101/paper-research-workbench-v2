...
description: >
  学术论文检索与下载工具，支持 IEEE Xplore 和万方数据平台。当用户提到搜索论文、查找文献、检索学术、
  IEEE搜索、万方搜索、下载论文PDF、导出引用、批量下载、文献综述、分章下载、学术调研、论文摘要等
  任何与学术文献检索相关的需求时，务必使用此 skill，不要用其他方式搜索论文。
...
> 「你当前是什么网络？① 机构网络（校园网/VPN）② 非机构网络（家庭/公共 WiFi）」

- ① → launch 模式，不需要 `--mode cdp`
- ② → 需要先启动 CDP Chrome + CARSI 登录，再加 `--mode cdp`

不要跳过这一步直接执行任何搜索、下载命令。
...
- **搜索结果摘要不完整**：IEEE 搜索返回的 snippet 是截断的，不可用于深入分析。必须用 `ieee-detail.js --arnumber <N>` 获取完整摘要后再做内容判断。
- **下载**需要认证——机构网络通过 IP 自动识别，非机构网络需要 CARSI SSO 登录。
...
所有支持 `--ids` 的脚本（分章下载、批量引用、批量下载）支持以下格式：

```
--ids "0,2,5"        # 单个索引
--ids "0-4"          # 范围（展开为 0,1,2,3,4）
--ids "0,2,4-6"      # 混合
--ids "0,1,0-2"      # 自动去重 → 0,1,2
```
...
| `ieee-parallel-detail.js` | IEEE 多论文并行详情 | `--arnumbers "n1,n2,n3"` `--parallel 3` `--mode launch\|cdp` |
| `wf-parallel-detail.js` | 万方多论文并行详情 | `--urls "url1,url2"` `--parallel 3` `--mode launch\|cdp` |
...
1. 询问浏览器偏好，运行：`node scripts/set-browser.js chrome`（或 `edge` / `firefox`）
...
| `references/workflow.md` | 标准工作流（搜索→下载步骤） |
